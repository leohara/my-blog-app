...
export function DebugCodeBlocks() {
  ...
  const analyzeCodeBlocks = () => {
    ...
    logs.push(`Found ${codeBlocks.length} elements with class 'code-block-wrapper'`);
    
    // Also check for figure elements
    const figures = document.querySelectorAll("figure[data-rehype-pretty-code-figure]");
    logs.push(`Found ${figures.length} figure elements with data-rehype-pretty-code-figure`);
    
    // Check each code block
    codeBlocks.forEach((block, index) => {
      const hasDataContent = !!(block as HTMLElement).dataset.codeContent;
      const dataContentLength = (block as HTMLElement).dataset.codeContent?.length || 0;
      const hasFigure = !!block.querySelector("figure");
      const hasCopyButton = !!block.querySelector(".copy-button-container");
      
      logs.push(`Block ${index + 1}: data-content=${hasDataContent} (${dataContentLength} chars), has-figure=${hasFigure}, has-button=${hasCopyButton}`);
    });
    
    // Check if any unwrapped figures exist
    const unwrappedFigures = Array.from(figures).filter(fig => !fig.closest(".code-block-wrapper"));
